...
interface AuthContextType {
  ...
  logout: () => Promise<void>;
  syncUserFromProfile: (info: Pick<User, 'id' | 'email' | 'name'>) => void;
...
export const AuthProvider = ({ children }: PropsWithChildren) => {
  ...
  const syncUserFromProfile = useCallback(
    (info: Pick<User, 'id' | 'email' | 'name'>) => {
      const uid = Number(info.id);
      setUser({
        ...info,
        id: Number.isFinite(uid) ? uid : info.id,
      });
    },
    [setUser]
  );
  ...
  const value = useMemo(
    ...
    [user, accessToken, isAuthenticated, isLoading, login, logout, syncUserFromProfile, clearLocalSession]
  );
