...
export default function MovieDetailPage() {
  ...
  const { data: movie, error, isLoading } = useFetch<Movie>(
  ...
  if (error || !movie) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-gray-100">
        ...
        <Link to="/" className="text-sm font-semibold text-blue-500 hover:underline">
          ← 홈으로
        </Link>
      </div>
    );
      ...
      <div className="mx-auto max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex flex-col gap-6 p-6 md:flex-row">
          ...
          <div className="flex flex-1 flex-col gap-5">
            ...
            <div>
              <h2 className="mb-1 text-sm font-bold text-gray-800">개봉일</h2>
              <p className="text-sm text-gray-600">{formatDate(movie.release_date, 'ko-KR')}</p>
            </div>

            <div>
              <h2 className="mb-2 text-sm font-bold text-gray-800">인기도</h2>
            ...
            <div>
              <h2 className="mb-2 text-sm font-bold text-gray-800">줄거리</h2>
