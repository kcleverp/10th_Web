...
import type { RequestPatchUser, ResponseMyInfo } from '../types/authType';
...
type PatchProfileRollback = {
  ...
  prevUser: { id: number; email: string; name: string };
};
...
const Mypage = () => {
  ...
  const { user, logout, isAuthenticated, isLoading: isAuthLoading, syncUserFromProfile } = useAuth();
  ...
  const patchMutation = useMutation<ResponseMyInfo, Error, void, PatchProfileRollback | undefined>({
    mutationFn: async () => {
